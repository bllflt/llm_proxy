...
async def process_caption_job(job_id: str) -> None:
    ...
    try:
        description = await analyze_image(model_client, job.image_file)
        if description is None:
            raise RuntimeError("Failed to generate description from image")
        if job.current_description:
            result = await compare_descriptions(
                model_client,
        ...
        await complete_job(job_id, result.model_dump())
        await submit_result(job.character_id, result.explanation, result.merge)
